...
// Allow up to 300s — Railway Playwright login + course scan takes 60-90s
export const maxDuration = 300;

export async function POST(req: NextRequest) {
  try {
    const body = await req.json();
    const res = await fetch(`${RAILWAY}/api/diksha/fetch-courses`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(body),
      // @ts-ignore — Node.js fetch signal workaround for long requests
      signal: AbortSignal.timeout(140000), // 140s internal timeout
    });
